...
layui.use(['table','form'], function(){
  ...
  table.render({
    ...
    ,cols: [[ //表头
      ...
      ,{field: 'selling_price',align:'center',width:'100px;', title: '出售价',templet: '#sexTpl2'}
	    	 ...
	    	 layer.open({
	    	        ...
	    	        ,yes: function(){
	    	        	...
	    	        	if(unit==null || unit==''){
	    	         	...
	    	         	$.post("${pageContext.request.contextPath}/goods/update.do",{type_id:type_id,id:id,name:name,model:model,purchasing_price:purchasing_price,selling_price:selling_price,min_num:min_num,producer:producer,remarks:remarks,unit:unit,numbers:numbers},function(result){
	    	         		if(result.success){
	    	         			layer.closeAll();
      ...
      layer.open({
        ...
        ,yes: function(){
        	...
        	if(unit==null || unit==''){
         	...
         	$.post("${pageContext.request.contextPath}/goods/save.do",{number:number,type_id:type_id,name:name,model:model,purchasing_price:purchasing_price,selling_price:selling_price,min_num:min_num,producer:producer,remarks:remarks,unit:unit},function(result){
         		if(result.success){
         			layer.closeAll();
 ...
 <div class="layui-form-item">
  ...
  <label class="layui-form-label" style="color: red;">出售价</label>
    <div class="layui-input-block" style="width: 100px;">
      <input type="text" onkeypress="if(!this.value.match(/^[\+\-]?\d*?\.?\d*?$/))this.value=this.t_value;else this.t_value=this.value;if(this.value.match(/^(?:[\+\-]?\d+(?:\.\d+)?)?$/))this.o_value=this.value" onkeyup="if(!this.value.match(/^[\+\-]?\d*?\.?\d*?$/))this.value=this.t_value;else this.t_value=this.value;if(this.value.match(/^(?:[\+\-]?\d+(?:\.\d+)?)?$/))this.o_value=this.value" onblur="if(!this.value.match(/^(?:[\+\-]?\d+(?:\.\d+)?|\.\d*?)?$/))this.value=this.o_value;else{if(this.value.match(/^\.\d+$/))this.value=0+this.value;if(this.value.match(/^\.$/))this.value=0;this.o_value=this.value}" name="price_min" id="selling_price" placeholder="￥" placeholder="请输入出售价" style="width: 250px;" autocomplete="off" class="layui-input">
    </div>
    </div>
	<div class="layui-form-item">
