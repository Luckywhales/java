	...
	//数据表格显示
	  table.render({
	    ...
	    ,cols: [[ //表头
	     ...
	     ,{field: 'remark',align:'center', title: '商品名称'}
	  ...
	  table.on('tool(test)', function(obj){
		    ...
		    if(obj.event === 'del'){
		        ...
		        layer.confirm('真的删除该订单吗？', function(index){
		            $.ajax({
		                url: "${pageContext.request.contextPath}/orderList/delete.do",
		                type: "POST",
		                data:{"id":data.id},
		                dataType: "json",
	  ...
	  form.on('submit(demo1)', function(data){
	    ...
	    if(data.field['supplier_id']== -1){
	     ...
	     $.ajax({
             ...
             type: "POST",
             data:{supplier_id:data.field['supplier_id'],paymoney:data.field['paymoney'],releaseStrPay:data.field['paydate'],state:data.field['state'],remark:data.field['remark'],num:data.field['num'],code:dh},
             dataType: "json",
             success: function(data){
                 if(data.success){
